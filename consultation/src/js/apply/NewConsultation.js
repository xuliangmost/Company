...
import {Button, DatePicker, Input, Table, Modal, Icon, Upload, message, Popconfirm} from 'antd';
...
export default class NewConsultation extends Component {
    constructor(props) {
        super(props);
        this.state = {
            consultationId: null,
                ...
                <a href={record.url} target="blank" className="apply_link">查看</a>
                            {
                                this.state.history1.statusId ?
                                    <Link to="" onClick={this.deleteFile.bind(this, record.id, index)}
                                          className="apply_link">删除</Link> : ""
                            }
...
/////////////////////////
    ...
    send() {
        if (this.state.saveCase) {
            if (this.state.saveAdvice) {
                if (this.state.getDoctor.length===0) {
                    alert("会诊医生未选择!");
                    return false
                }
                axios.request({
                    ...
                    params: {
                        ...
                        aId: this.state.aId.toString(),
                        hId: this.state.hospitalId.toString(),
    ...
    checkUserId() {
        let that = this;
        let data = this.state.getData.consultation.phone;
        axios.request({
    ...
    saveConsultation() {//保存会诊
        let postConsultation = this.state.getData.consultation;

        if (tool.isEmpty(postConsultation.consultationName)) {
            alert("会诊名称不能为空!");
            return false
        }
        if (tool.isEmpty(postConsultation.startTime)) {
            alert("会诊时间不能为空!");
            return false
        }

        if (!tool.mobileValidate(postConsultation.phone)) {
            alert("手机号不能为空或手机号格式错误!");
            return false
        }
        if (tool.isEmpty(postConsultation.username)) {
            alert("会诊对象不能为空!");
            return false
        }

        if (!tool.cardValidate(postConsultation.identification)) {
            alert("身份证号不能为空或身份证号格式填写错误!");
            return false
        }
        if (tool.isEmpty(postConsultation.birthday)) {
            alert("出生日期不能为空!");
            return false
        }
        if (!postConsultation.birthday) {
            postConsultation.birthday = startTime
        }
        postConsultation.userId = this.state.userId ? this.state.userId.toString().toString() : "";
        delete postConsultation.id;
        let that = this;
        axios.request({
            url: "/api/conference/add/consultation",
            method: 'POST',
            data: postConsultation,
            headers: {
                'Authorization': 'Bearer ' + token,
                'Content-Type': 'application/json'
            },
            ...
            if (response.data.result) {
                postConsultation.id = response.data.id;
                let getData = JSON.parse(JSON.stringify(that.state.getData));
                getData.consultation = postConsultation;
                that.setState({
            ...
            axios.request({
                ...
                if (response.data.code === 200) {
                    ...
                    that.setState({
                        ...
                        history1: history1,
                        history2: history2,
                        history2Index: that.state.history2Index - 1,
                        data,
                        saveAdvice
                    })
                }
            ...
            <div style={this.state.showPrescription ? Hidden : this.state.isShow ? Hidden : null} className="newHidden">
                            ...
                            <ul>
                                <li>
                                    ...
                                    <DatePicker showTime format={dateFormat}
                                                onChange={this.changePrescriptionTime.bind(this)} size="large"
                                                placeholder="必填"/>
                                </li>
                                <li>
                                    <span>开方医生姓名</span>
                                    <Input value={this.state.centerPrescription.doctorName}
                                           onChange={this.changeDoctorName.bind(this)} size="large"
                                           placeholder="开方医生姓名"/>
                                </li>

                                <li>
                                    <span>药品名称</span>
                                    <Input value={this.state.centerPrescription.medicineTime}
                                           onChange={this.changeMedicineTime.bind(this)} size="large"
                                           placeholder="药品名称"/>
                                </li>

                                <li>
                                    <span>总量</span>
                                    <Input value={this.state.centerPrescription.total}
                                           onChange={this.changeTotal.bind(this)} size="large" placeholder="总量"/>
                                </li>
                                <li>
                                    <span>单次用量</span>
                                    <Input value={this.state.centerPrescription.singleDose}
                                           onChange={this.changeSingleDose.bind(this)} size="large" placeholder="单次用量"/>
                                </li>

                                <li>
                                    <span>频次</span>
                                    <Input value={this.state.centerPrescription.frequency}
                                           onChange={this.changeFrequency.bind(this)} size="large" placeholder="频次"/>
                                </li>

                            </ul>
                            <Button onClick={this.closePrescription.bind(this)} className="transfer_btn1"
                                    type="primary">保存处方</Button>
                            <Button onClick={this.cancelSaveCF.bind(this)} className="transfer_btn1"
                                    type="primary">取消保存</Button>
                        </div>
                    </div> : ""
                }
                ...
                <div className="cnsultation_top">
                    ...
                    <ul className="search_ul">
                        <li>
                            ...
                            <Input readOnly value={this.state.getData.consultation.hospital} className="search_input"
                                   size="large" placeholder="隶属医院"/>
                        </li>
                        <li>
                            <span className="most_flex">会诊申请人</span>
                            <Input readOnly value={this.state.getData.consultation.applicant} className="search_input"
                                   size="large" placeholder="会诊申请人"/>
                        </li>
                        <li>
                            <span className="most_flex">会诊名称</span>
                            <Input value={this.state.getData.consultation.consultationName}
                                   onChange={this.changeConsultationName.bind(this)} className="search_input"
                                   size="large" placeholder="必填"/>
                        </li>
                        <li>
                            <span className="most_flex">会诊时间</span>{/*这里要加上一个判断， 判断不为空*/}
                            <DatePicker placeholder="必填" showTime format={dateFormat} size="large"
                                        className="search_input" onChange={this.changesStartTime.bind(this)}/>
                    ...
                    <ul className="search_ul">
                        <li>
                            ...
                            <Input value={this.state.getData.consultation.phone} onBlur={this.checkUserId.bind(this)}
                                   className="search_input" size="large" placeholder="必填"
                                   onChange={this.changePhone.bind(this)}/>
                        </li>
                        <li>
                            <span className="most_flex">会诊对象</span>
                            <Input value={this.state.getData.consultation.username} className="search_input"
                                   size="large" placeholder="必填" onChange={this.changeUsername.bind(this)}/>
                        </li>

                        <li>
                            <span className="most_flex">身份证号</span>
                            <Input value={this.state.getData.consultation.identification} className="search_input"
                                   size="large" placeholder="必填" onChange={this.changeIdentification.bind(this)}/>
                        </li>
                        <li>
                    ...
                    <ul className="search_ul">
                        <li>
                            ...
                            <Input value={this.state.getData.consultation.famliyName} className="search_input"
                                   size="large" placeholder="陪护家属" onChange={this.changeFamliyName.bind(this)}/>
                        </li>
                        <li>
                            <span className="most_flex">手机号</span>
                            <Input value={this.state.getData.consultation.familyPhone} className="search_input"
                                   size="large" placeholder="手机号" onChange={this.changeFamilyPhone.bind(this)}/>
                        </li>
                    ...
                    <ul className="search_ul2">
                        <li>
                            ...
                            <Input value={this.state.getData.consultation.content} className="search_input"
                                   type="textarea" rows={4} onChange={this.changeContent.bind(this)}/>
                        </li>
                    </ul>
                ...
                <div className="btn_save">
                    <div className="btn_save_index">
                        <Button disabled={this.state.dis} onClick={this.saveConsultation.bind(this)}
                                className="btn_save_index_2" type="primary">保存会诊</Button>
                    </div>
                </div>
                ...
                <div className="cnsultation_bottom">
                    ...
                    <div className="history_detail">{/*这里循环一个state，点击病历就切换this.setState   点击新增就让新增的这个id去setState */}
                        <ul className="search_ul">
                            <li>
                                ...
                                <Input value={this.state.history1.sn} className="search_input" size="large"
                                       placeholder="必填" onChange={this.changeSn.bind(this)}/>
                            </li>
                            <li>
                            </li>
                        ...
                        <ul className="search_ul">
                            <li>
                                ...
                                <Input value={this.state.history1.hospital} className="search_input" size="large"
                                       placeholder="必填" onChange={this.changeHospital.bind(this)}/>
                            </li>
                            <li>
                                <span className="most_flex">主治医生</span>
                                <Input value={this.state.history1.doctor} className="search_input" size="large"
                                       placeholder="主治医生" onChange={this.changeDoctor.bind(this)}/>
                            </li>
                            <li>
                                <span className="most_flex">病例名称</span>
                                <Input value={this.state.history1.name} className="search_input" size="large"
                                       placeholder="病例名称" onChange={this.changeName.bind(this)}/>
                            </li>
                            <li>
                                <span className="most_flex">诊治日期</span>
                                <DatePicker allowClear={false}
                                            value={moment(this.state.history1.diagnosisTime, dateFormat)}
                                            format={dateFormat} size="large" placeholder="必填" className="search_input"
                                            onChange={this.changeDagnosisTime.bind(this)}/>
                            </li>
                        </ul>
                        ...
                        <ul className="search_ul2">
                            <li>
                                ...
                                <Input onChange={this.changeDiagnosis.bind(this)} value={this.state.history1.diagnosis}
                        ...
                        this.state.history2 ? <div className="prescribeDetail">
                            <ul className="search_ul">
                                <li>
                                    ...
                                    <Input value={this.state.history2.hospital ? this.state.history2.hospital : ""}
                                           onChange={this.changeAdviceHospital.bind(this)} className="search_input"
                                           size="large" placeholder="必填"/>
                                </li>
                                <li>
                                    <span className="most_flex">医嘱医生</span>
                                    <Input value={this.state.history2.doctor ? this.state.history2.doctor : ""}
                                           onChange={this.changeAdviceDoctor.bind(this)} className="search_input"
                                           size="large" placeholder="必填"/>
                                </li>
                                <li>
                                    ...
                                    <DatePicker allowClear={false}
                                                ...
                                                format={dateFormat} placeholder="必填" size="large"
                                                className="search_input" onChange={this.changeAdviceTime.bind(this)}/>
                                </li>
                                <li>
                                </li>
                            </ul>
                            <ul className="search_ul2">
                                <li>
                                    ...
                                    <Input value={this.state.history2.advice ? this.state.history2.advice : ""}
                                           onChange={this.changeAdvice.bind(this)} className="search_input"
                    ...
                    <div className="record">
                        ...
                        <Table rowKey="id" className="fileList" columns={this.state.fileListColumns}
